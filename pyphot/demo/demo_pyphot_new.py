...
#For a single spectrum, synthetic photometry with errors
#   This is accomplished by generating n_realisations realisations of the spectrum, and treating this
#   as a grid of spectra which is fed to extractSEDs.
#Input spectrum
n_realisations = 1000
spec = np.loadtxt(demodir+'IRC+10216_ISO_SWS.dat', comments='#', usecols=(0,1,2))
#Retrieve filter responses interpolated over input wavelength grid
filterList = filterLibrary.load_filters(filterNames, interp=True, lamb=spec[:,0]*pyphot.unit['micron'])
specgrid = np.zeros((n_realisations, len(spec[:,0])))
for i in range(n_realisations):
    specgrid[i,:] = np.random.normal(loc=spec[:,1], scale=spec[:,2])
cls, seds = pyphot.extractSEDs(spec[:,0], specgrid, filterList, Fnu=True, absFlux=False)
fphot = np.mean(seds, axis=0)
dfphot = np.std(seds, axis=0)
